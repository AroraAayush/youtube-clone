...
function ChannelCard({channel,channelPage=false}) {
  return (
    ...
    <div className={`${channelPage==false?"":"-mt-20"} shadow-lg cursor-pointer flex flex-col justify-center items-center px-3 min-w-80`}>
      ...
      {channel?.statistics && <span className='text-base font-light'>{Number(channel?.statistics?.subscriberCount).toLocaleString()} subscribers</span>
      }
    </div>
    </Link>
